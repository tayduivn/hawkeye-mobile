...
export class TaskService {
  ...
  getTaskList(): Observable<contracts> {                    //通过用户信息获取任务列表
  ...
  delTaskById(params: inspecParams) {                       //通过任务id删除任务
    return this.http.delete({ url: '/task/task-delete', params: { id: params.id } })
  }

  upDateTask(params: upDateTaskParams) {                    //修改任务
    return this.http.post({ url: '/task/task-update', params: { id: params.id, user_id: params.user_id, contract_id: params.contract_id, date: params.date, count: params.count } })
  }

  addTask(params: upDateTaskParams) {                       //添加任务
    return this.http.post({ url: '/task/task-add', params: { user_id: params.user_id, contract_id: params.contract_id, date: params.date, count: params.count } })
  }

  getTaskById(taskId: number): Observable<contracts> {        //通过任务id获取任务详情  sku-列表
    return this.http.get({ url: '/task/task-sku-list', params: { task_id: taskId } })
  }

  toInspection(params: inspecParams): Observable<contracts> {       //验货  （sku）
    return this.http.get({ url: '/task/task-sku-view', params: { task_id: params.id, sku: params.sku,contract_id:params.contract_id } })
  }

  getAccessbyTaskAndFac(params:getPoParams):Observable<Array<any>>{          //验货   （acc）
    return this.http.get({url:'/task/task-acc-view',params:{task_id:params.task_id,contract_id:params.contract_id}})
  }

  getPoListByTaskAndFactory(params: getPoParams): Observable<contracts> {    //po列表
    return this.http.get({ url: '/task/task-factory-contract', params: { task_id: params.task_id, factory_group: params.factory_group } })
  }

  getSkuListByPoAndTask(task_id: number, contract_id: number) {
    return this.http.get({ url: '/task/contract-sku-list', params: { task_id: task_id, contract_id: contract_id } })
  }

  uploadTask(data: any):Observable<any>{                         //上传任务
    return this.http.post({ url: '/task/task-add', params: data })
  }

  getScheduleList(contract_id:number):Observable<any>{                                      //进度列表
    return this.http.get({url:'/schedule/'+contract_id})
  } 

  updateSchedule(update:any){                                                     //更新进度
