...
export class HttpService {
    constructor(private http: HttpClient, private loading: LoadingService, private userInfo: UserInfoService) {}
    ...
    delete(params: publicParams, showLoading?: boolean): Observable<any> {
        !showLoading && this.loading.setLoading(true);
        return this.http.delete(environment.apiUrl + params.url + '/' + params.params, {
            headers: {
                Authorization: this.userInfo.info ? `Bearer ${this.userInfo.info.api_token}` : undefined,
            },
        });
    ...
    post(params: publicParams, showLoading?: boolean): Observable<any> {
        ...
        return this.http.post(environment.apiUrl + params.url, JSON.stringify(params.params), {
            headers: obj,
        });
    }
}
