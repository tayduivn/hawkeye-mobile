...
export class SkuDetailPage implements OnInit {
  ...
  constructor(
    ...
    this.addDescParams.apply_id = this.storage.get(
      "CURRENT_INSPECT_CONTRACT"
    ).id;
  }
  ...
  ionViewWillEnter(){
    this.data.inspection_task_desc = (this.data.inspection_task_desc
      ? this.data.inspection_task_desc
      : [""]);
  }
  ...
  addInspectDesc() {
    ...
    this.inspectCtrl
      .addInspectionTaskDesc(this.addDescParams)
      .subscribe(res => {
