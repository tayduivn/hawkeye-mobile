...
export class InspectFactoryComponent implements OnInit {
  ...
  ngOnInit() {
    const IMPLEMENT_META_DATA = this.storage.get(
  ...
  getData() {
    this.implementInspect
      ...
      .subscribe(res => {
        ...
        setTimeout(() => {
          if (
            res.factory_data.environments &&
            res.factory_data.environments.length
          ) {
            // tslint:disable-next-line: prefer-for-of
            for (let i = 0; i < res.factory_data.environments.length; i++) {
              (this.factoryModel.get('environments') as FormArray).push(
                new FormControl('')
              );
            }
            (this.factoryModel.get('environments') as FormArray).setValue(
              res.factory_data.environments
            );
          }

          if (
            res.factory_data.sampleRoom &&
            res.factory_data.sampleRoom.length
          ) {
            // tslint:disable-next-line: prefer-for-of
            for (let i = 0; i < res.factory_data.sampleRoom.length; i++) {
              (this.factoryModel.get('sampleRoom') as FormArray).push(
                new FormControl('')
              );
            }
            (this.factoryModel.get('sampleRoom') as FormArray).setValue(
              res.factory_data.sampleRoom
            );
          }

          if (
            res.factory_data.factoryOther &&
            res.factory_data.factoryOther.length
          ) {
            // tslint:disable-next-line: prefer-for-of
            for (let i = 0; i < res.factory_data.factoryOther.length; i++) {
              (this.factoryModel.get('factoryOther') as FormArray).push(
                new FormControl('')
              );
            }
            (this.factoryModel.get('factoryOther') as FormArray).setValue(
              res.factory_data.factoryOther
            );
          }
        }, 2000);
