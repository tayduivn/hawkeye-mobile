...
export class InspectSettingBoxComponent implements OnInit {
  ...
  constructor(private modalService:ModalController,
              ...
              private inspectService:InspectionService,private effectCtrl:PageEffectService) { }
  ...
  enter(){
    let params = {
      ...
      apply_id:(this.type=='skuList'||this.type=='skuDetail') ? this.storage.get('CURRENT_INSPECT_CONTRACT').id : this._contract.id
    }
    if(this.type=='skuList'||this.type=='skuDetail'){
      this.inspectService.addInspectionTaskDesc({apply_id:params.apply_id,
        sku:this._contract.sku,
        is_know_demand:params.is_know_demand,is_inspected_product:params.is_inspected_product})
          .subscribe(data => {
            if(data.status == 1){
      ...
      this.inspectService.inspectSetting(params)
      .subscribe(data => {
        if(data.status == 1){
          this.modalService.dismiss({
            'refresh': this._contract
